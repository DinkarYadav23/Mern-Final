...
export default function EditableWorkoutRow({ workout, setError }) {
  ...
  const handleSubmit = (e) => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`${process.env.REACT_APP_URL}/api/workout/${workout._id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleClickDelete = () => {
    fetch(`${process.env.REACT_APP_URL}/api/workout/${workout._id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${user.token}`,
      },
    ...
    <tr key={workout._id} ref={formRef}>
      ...
      <td>
        {isEdit ? (
          ...
          <span className="dot">
            {workout.exercise[0].name} <Dot color={workout.exercise[0].color} />{' '}
          </span>
        )}
      </td>
      <td>
        {isEdit ? (
          ...
          <span>
            {workout.time} {workout.time === 1 ? 'minute' : 'minutes'}
          </span>
        )}
      </td>
