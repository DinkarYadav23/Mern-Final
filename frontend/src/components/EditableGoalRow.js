...
export default function EditableGoalRow({goal, setError, exercises}) {
    ...
    useEffect(() => {
        fetch(`${process.env.REACT_APP_URL}/api/workout`, {
            headers: {
    ...
    const handleSubmit = (e) => {
        ...
        const fetchData = async () => {
            ...
            const response = await fetch(`${process.env.REACT_APP_URL}/api/goals/${goal._id}`, {
                method: 'PATCH', headers: {
                    'Content-Type': 'application/json', Authorization: `Bearer ${user.token}`,
                }, body: JSON.stringify({
                    goal_type: goalType, time, frequency, exercise,
                }),
            });
    ...
    const handleClickDelete = () => {
        fetch(`${process.env.REACT_APP_URL}/api/goals/${goal._id}`, {
    ...
    return (<tr ref={formRef}>
            <td>
                {isEdit ? (<ExerciseSelect
                        exercises={exercises}
                        onChange={handleChangeExercise}
                        defaultValue={{label: goal.exercise[0].name, value: goal.exercise[0]._id}}
                    />) : (<span className="dot">
            {goal.exercise[0].name} <Dot color={goal.exercise[0].color}/>{' '}
          </span>)}
            </td>
            <td>
                {isEdit ? (<form className="editable-form" onSubmit={() => {
                    }}>
                    ...
                    </form>) : (<span>{formattedGoalType}</span>)}
            </td>
            <td className={goalType === 'time' ? 'editable-form-time' : null}>
                {isEdit ? (<form className="editable-form" onSubmit={() => {
                    }}>
                        {goalType === 'time' && <TimeInput time={time} setTime={setTime} noLabel={true}/>}
                        {goalType === 'frequency' && (<div className="form-group time-form-group">
                                <input
            ...
            {goal.goal_type === 'time' ? `${goal.time} minutes` : `${goal.frequency} times`}
          </span>)}
            </td>
            ...
            {remaining} {goalType === 'time' ? (remaining === 1 ? 'minute' : 'minutes') : (remaining === 1 ? 'time' : 'times')}
                    ...
                    {isEdit ? <button className="editable-form-btn">Save</button> : null}
                </form>
            </td>
